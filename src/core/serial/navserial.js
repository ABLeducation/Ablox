...
class NavSerial extends BaseSerial {
  constructor() {
    ...
    this._dataHandler = (buff) => {
      ...
      if (this.DEBUG)
        console.log(
          'read',
          buff.toString(this.encoding),
          buffer.Buffer.from(buff).toString('hex')
        );
  ...
  async _initSerial() {
    const { usbProductId, usbVendorId } = JSON.parse(
    ...
    const device = devices.find((d) => {
      ...
      return (
        usbProductId === info.usbProductId && usbVendorId === info.usbVendorId
      );
    });
