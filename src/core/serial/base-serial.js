...
class BaseSerial extends EventEmitter {
  ...
  async listDevices() {
    const isDevices = await Promise.all(
      this.devices.map((device) => this.isDevice(device.value))
    );
  ...
  _transSignal(sig) {
    ...
    Object.keys(signalMap).forEach((i) => {
      ...
      if (typeof trans[signalMap[i]] === 'boolean')
        trans[i] = trans[signalMap[i]];
    });
    return trans;
  }
  ...
  async write(message, encoding = null, cb = () => {}) {
    ...
    try {
      await this.writeBuff(
        typeof message === 'string'
          ? buffer.Buffer.from(message, encoding || this.encoding)
          : message
      );
    } catch (err) {
      cb?.(err);
